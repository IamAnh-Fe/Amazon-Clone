...
const productController = {
    ...
    getAllProduct: asyncHandler(async (req, res) => {
        const features = new APIfeatures(Product.find(), req.query).paginating().sorting().searching().filtering();
    ...
    getProduct: asyncHandler(async (req, res) => {
        ...
        const result = await Promise.allSettled([
            features.query,
            Product.countDocuments({ category: req.params.category }), //count number of product.
        ]);
        const product = result[0].status === 'fulfilled' ? result[0].value : [];
    ...
    findCategory: asyncHandler(async (req, res) => {
        const getCategory = await Product.find({ category: 'headsets' });
        if (getCategory) {
    ...
    postProduct: asyncHandler(async (req, res) => {
        ...
        // }
        if (saveProduct) {
            return res.status(200).send({ message: 'New Product Created', saveProduct });
        } else {
